...
[Go to the question](https://www.hackerrank.com/challenges/weather-observation-station-12/problem?isFullScreen=true)
...
If you solve Weather-Observation-Station question 11, you'll notice that this is almost exactly the same questions with a minor change: instead of querying names that do not start OR end with vowels, we are looking for words that do not start AND end with vowels. You can write two different codes to answer this question. Depending on the method you choose, this question tests whether you know:
...
When you see a question like this, I recommend you to chop up into bite size pieces. Let's start this condition: **either do not start with vowels AND do not end with vowels**

1. Get rid of the noise first. Suppose that I substitute "do not start with vowels" with X and "do not end with vowels" with Y. The expression becomes much easier to understand: **X AND Y". So, you'll have conditions X and Y and you'll connect them with an AND clause. 
...
As you see, we explicitly defined condition for names that do not start and end with vowels. Then, we connected these two with an AND clause. A% means starting with A. %A means ending with A. Easy peasy, lemon squeezy.
